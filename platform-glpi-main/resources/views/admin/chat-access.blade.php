...
@section('title', 'Accès chat - L2T Support')
  ...
  body.innerHTML = sorted.map(u => {
    ...
    return `
      <tr>
        ...
        <td>${switchHtml(Boolean(u.can_reply_whatsapp), u.id, 'can_reply_whatsapp', self)} <span class="ca-mode">${u.can_reply_whatsapp ? 'Peut répondre' : 'Lecture seule'}</span></td>
        <td>${mode}</td>
        <td>
          <div class="ca-actions">
            <button class="ca-action" data-user-ro="${esc(u.id)}" data-readonly="1" ${self ? 'disabled' : ''}>Lecture seule</button>
