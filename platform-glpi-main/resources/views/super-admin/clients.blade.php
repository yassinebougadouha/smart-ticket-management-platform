...
@section('page-title','Gérer les clients')
    ...
    <div class="card p-4" style="background:linear-gradient(135deg,var(--color-primary) 0%,var(--color-secondary) 100%);border:none;box-shadow:0 10px 30px -5px rgba(0,0,0,0.15);">
      <div class="d-flex align-items-center">
        ...
        <div>
          ...
          <p class="text-white text-sm mb-0 opacity-9" style="font-weight:500;">{{ $countAll }} clients enregistrés dans la plateforme</p>
        </div>
      </div>
    </div>
  </div>
...
<div class="card">
  <div class="card-body p-3">
    ...
    <div class="d-flex align-items-start justify-content-between flex-wrap gap-3 mb-3">
      <div>
        <div class="seg-tabs">
          ...
          <a href="{{ route('super-admin.clients') }}?client_type=user"
      ...
      <div class="cl-search" style="min-width:240px;max-width:320px;">
        ...
        <input type="text" id="clientSearch" placeholder="Nom, email, téléphone..." value="{{ request('search') }}" autocomplete="off">
        <button id="clearBtn" type="button" onclick="clearSearch()" style="background:none;border:none;cursor:pointer;padding:0;display:{{ request('search') ? 'block' : 'none' }}">
    ...
    <div style="border:1px solid #f1f5f9;border-radius:12px;overflow:hidden;">
      ...
      @endphp
      <div class="cl-item" onclick="window.location.href='{{ route('super-admin.clients.show', $client->id) }}'" style="cursor:pointer;">
        <div class="cl-avatar">
          @if($client->avatar)<img src="{{ asset('storage/'.$client->avatar) }}" style="width:100%;height:100%;object-fit:cover;" alt="">
      ...
      <div class="text-center py-5">
        ...
        @if(request('search') || request('client_type'))
          <a href="{{ route('super-admin.clients') }}" class="btn btn-sm btn-outline-secondary">Réinitialiser</a>
        @endif
      </div>
      @endforelse
    </div>
...
var CLIENTS_DATA = {
  ...
  {{ $c->id }}: {
    id:{{ $c->id }}, name:@json($c->name), email:@json($c->email),
    phone:@json($c->phone ?? null), phone_mobile:@json($c->phone_mobile ?? null),
    whatsapp:@json($c->whatsapp ?? null), teams_email:@json($c->teams_email ?? null),
    type:@json($c->client_type), glpi_id:@json($c->glpi_user_id ?? null),
    tickets:{{ $c->tickets_count }}, active:{{ $c->is_active ? 'true' : 'false' }},
    ...
    first_name:@json($c->first_name ?? null), last_name:@json($c->last_name ?? null),
