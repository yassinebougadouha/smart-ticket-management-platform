...
@section('title', 'Chat - Supervision - L2T Support')
    ...
    <div class="c-panel" id="panelClients">
      <div class="c-panel-head">
        ...
        <div class="c-mode-tabs">
          <button class="c-mode-tab active-client" id="btnModeClient" onclick="switchMode('client')">
            ...
            Clients
            <span class="c-mode-cnt" id="cntClients">{{ $allClients->count() }}</span>
          </button>
          <button class="c-mode-tab" id="btnModeAdmin" onclick="switchMode('admin')">
            <span class="c-mode-icon">🛡️</span>
            Admins
            <span class="c-mode-cnt" id="cntAdmins">{{ $allAdmins->count() }}</span>
          </button>
        </div>
      ...
      <div class="c-panel-body" id="clientList">
        ...
        <div id="clientItems">
          ...
          <div class="c-client-item" data-uid="{{ $client->id }}" data-mode="client"
            ...
            <div class="c-client-avatar" style="background: linear-gradient(135deg,#0ea5e9,#0284c7);">
              @if($client->avatar)<img src="{{ asset('storage/'.$client->avatar) }}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">
              @else{{ strtoupper(substr($client->name,0,2)) }}@endif
            </div>
            <div class="c-client-info">
              <div class="c-client-name">{{ $client->name }}</div>
              <div class="c-client-email">{{ $client->email }}</div>
            </div>
          </div>
          @endforeach
        </div>

        {{-- ADMIN items (hidden initially) --}}
        <div id="adminItems" style="display:none;">
          @foreach($allAdmins as $admin)
          <div class="c-client-item" data-uid="{{ $admin->id }}" data-mode="admin"
            ...
            <div class="c-client-avatar" style="background: linear-gradient(135deg,#8b5cf6,#6d28d9);">
              @if($admin->avatar)<img src="{{ asset('storage/'.$admin->avatar) }}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">
              @else{{ strtoupper(substr($admin->name,0,2)) }}@endif
            </div>
            <div class="c-client-info">
              <div class="c-client-name">{{ $admin->name }}</div>
              <div class="c-client-email">{{ $admin->email }}</div>
            </div>
          </div>
          @endforeach
        </div>
