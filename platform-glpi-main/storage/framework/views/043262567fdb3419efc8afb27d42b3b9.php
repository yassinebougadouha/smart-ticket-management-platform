<?php $__env->startSection('title','Gérer les clients'); ?>
<?php $__env->startSection('page-title','Gérer les clients'); ?>
    ...
    <div class="card p-4" style="background:linear-gradient(135deg,var(--color-primary) 0%,var(--color-secondary) 100%);border:none;box-shadow:0 10px 30px -5px rgba(0,0,0,0.15);">
      <div class="d-flex align-items-center">
        <div class="bg-white border-radius-lg p-3 me-4 shadow-sm d-flex align-items-center justify-content-center" style="width:72px;height:72px;border-radius:20px !important;">
          <i class="material-symbols-rounded" style="font-size:42px;color:var(--color-primary);">group</i>
        </div>
        <div>
          <h4 class="text-white font-weight-bolder mb-1">Gérer les clients</h4>
          <p class="text-white text-sm mb-0 opacity-9" style="font-weight:500;"><?php echo e($countAll); ?> clients enregistrés dans la plateforme</p>
        </div>
      </div>
    </div>
  </div>
...
<div class="card">
  <div class="card-body p-3">
    ...
    <div class="d-flex align-items-start justify-content-between flex-wrap gap-3 mb-3">
      <div>
        <div class="seg-tabs">
          <a href="<?php echo e(route('super-admin.clients')); ?>"
          ...
          <a href="<?php echo e(route('super-admin.clients')); ?>?client_type=user"
      ...
      <div class="cl-search" style="min-width:240px;max-width:320px;">
        ...
        <input type="text" id="clientSearch" placeholder="Nom, email, téléphone..." value="<?php echo e(request('search')); ?>" autocomplete="off">
        <button id="clearBtn" type="button" onclick="clearSearch()" style="background:none;border:none;cursor:pointer;padding:0;display:<?php echo e(request('search') ? 'block' : 'none'); ?>">
    ...
    <div style="border:1px solid #f1f5f9;border-radius:12px;overflow:hidden;">
      ...
      ?>
      <div class="cl-item" onclick="window.location.href='<?php echo e(route('super-admin.clients.show', $client->id)); ?>'" style="cursor:pointer;">
        <div class="cl-avatar">
          <?php if($client->avatar): ?><img src="<?php echo e(asset('storage/'.$client->avatar)); ?>" style="width:100%;height:100%;object-fit:cover;" alt="">
      ...
      <div class="text-center py-5">
        ...
        <?php if(request('search') || request('client_type')): ?>
          <a href="<?php echo e(route('super-admin.clients')); ?>" class="btn btn-sm btn-outline-secondary">Réinitialiser</a>
        <?php endif; ?>
      </div>
      <?php endif; ?>
    </div>
...
var CLIENTS_DATA = {
  ...
  <?php echo e($c->id); ?>: {
    id:<?php echo e($c->id); ?>, name:<?php echo json_encode($c->name, 15, 512) ?>, email:<?php echo json_encode($c->email, 15, 512) ?>,
    phone:<?php echo json_encode($c->phone ?? null, 15, 512) ?>, phone_mobile:<?php echo json_encode($c->phone_mobile ?? null, 15, 512) ?>,
    whatsapp:<?php echo json_encode($c->whatsapp ?? null, 15, 512) ?>, teams_email:<?php echo json_encode($c->teams_email ?? null, 15, 512) ?>,
    type:<?php echo json_encode($c->client_type, 15, 512) ?>, glpi_id:<?php echo json_encode($c->glpi_user_id ?? null, 15, 512) ?>,
    tickets:<?php echo e($c->tickets_count); ?>, active:<?php echo e($c->is_active ? 'true' : 'false'); ?>,
    ...
    first_name:<?php echo json_encode($c->first_name ?? null, 15, 512) ?>, last_name:<?php echo json_encode($c->last_name ?? null, 15, 512) ?>,
...
<div id="clientDrawer">
  <div style="background:linear-gradient(135deg,var(--color-primary),var(--color-secondary));padding:20px;flex-shrink:0;">
    ...
    <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
      <div style="background:rgba(255,255,255,0.15);border-radius:10px;padding:10px;text-align:center;"><p id="drawerTickets" style="color:white;font-size:22px;font-weight:800;margin:0;"></p><p style="color:rgba(255,255,255,0.7);font-size:10px;margin:0;">Tickets</p></div>
      <div style="background:rgba(255,255,255,0.15);border-radius:10px;padding:10px;text-align:center;"><p id="drawerStatus" style="color:white;font-size:14px;font-weight:700;margin:0;"></p><p style="color:rgba(255,255,255,0.7);font-size:10px;margin:0;">Statut</p></div>
      <div style="background:rgba(255,255,255,0.15);border-radius:10px;padding:10px;text-align:center;"><p id="drawerDate" style="color:white;font-size:12px;font-weight:600;margin:0;"></p><p style="color:rgba(255,255,255,0.7);font-size:10px;margin:0;">Inscrit le</p></div>
    </div>
  </div>
  ...
  <div class="drawer-body">
    ...
    <div class="drawer-section">
      ...
      <button type="button" id="saveTypeBtn" onclick="saveClientType()"
              ...
              disabled>
        <i class="material-symbols-rounded me-1" style="font-size:16px;vertical-align:middle;">save</i>Enregistrer le type
      </button>
    </div>
    ...
    <div>
      ...
      <div id="drawerTicketsList" style="max-height:320px;overflow-y:auto;" onwheel="event.stopPropagation();">
        <div style="text-align:center;padding:20px;color:#94a3b8;"><div style="width:24px;height:24px;border:3px solid var(--color-primary);border-top-color:transparent;border-radius:50%;animation:spin .8s linear infinite;margin:0 auto 8px;"></div>Chargement…</div>
      </div>
    </div>
  </div>

  <div style="padding:14px 20px;border-top:1px solid #e2e8f0;display:flex;gap:8px;flex-shrink:0;">
    <a id="drawerViewBtn" href="#" class="btn btn-sm mb-0 flex-fill text-white" style="background:linear-gradient(135deg,var(--color-primary),var(--color-secondary));">
      <i class="material-symbols-rounded me-1" style="font-size:14px;vertical-align:middle;">confirmation_number</i>Tous les tickets
    </a>
    <button id="drawerChatBtn" type="button" class="btn btn-sm btn-outline-primary mb-0 flex-fill" style="display:none;">
      <i class="material-symbols-rounded me-1" style="font-size:14px;vertical-align:middle;">chat</i>Conversation
    </button>
  </div>
</div>
...
var STATUS_META = {
  'pending':{color:'#f59e0b',label:'En attente'},'in_progress':{color:'#3b82f6',label:'En cours'},
  'resolved':{color:'#22c55e',label:'Résolu'},'closed':{color:'#94a3b8',label:'Clôturé'},
  'local':{color:'#f59e0b',label:'En attente'},'synced':{color:'#3b82f6',label:'En cours'},
};
var TYPE_META = { 'client':{icon:'🟣',label:'Client'}, 'user':{icon:'🟠',label:'Nouveau'} };
...
function openDrawer(id) {
  ...
  var info = [infoRow('🆔','ID Plateforme', '#'+d.id)];
  if (d.first_name || d.last_name) info.push(infoRow('👤','Prénom / Nom', (d.first_name||'') + ' ' + (d.last_name||'')));
  if (d.birthday) info.push(infoRow('🎂','Naissance', d.birthday + (d.age ? ' ('+d.age+' ans)' : '')));
  if (d.gender) { var gl = {male:'♂ Homme',female:'♀ Femme',other:'⚧ Autre'}; info.push(infoRow('⚧','Genre', gl[d.gender]||d.gender)); }
  if (d.glpi_id) info.push(infoRow('🔗','ID GLPI', d.glpi_id));
  ...
  var ticketsUrl = '/super-admin/tickets?client_id=' + id;
...
function loadTickets(clientId) {
  ...
  fetch('/super-admin/clients/'+clientId+'/tickets', {headers:{'Accept':'application/json','X-CSRF-TOKEN':_csrf}})
  .then(function(r) { return r.json(); })
  .then(function(data) {
    if (!data.tickets || data.tickets.length === 0) { list.innerHTML = '<div style="text-align:center;padding:20px;color:#94a3b8;font-size:13px;">Aucun ticket pour ce client</div>'; return; }
    var html = '';
    data.tickets.forEach(function(t) {
      var sm = STATUS_META[t.sync_status] || {color:'#94a3b8',label:'Inconnu'};
      html += '<a href="/admin/tickets/'+t.id+'" target="_blank" class="ticket-card">' +
  ...
  .catch(function() { list.innerHTML = '<div style="text-align:center;padding:20px;color:#ef4444;font-size:13px;">⚠️ Erreur de chargement</div>'; });
}
...
function saveClientType() {
  ...
  fetch('/super-admin/clients/'+_cid+'/type', {
    method:'POST', headers:{'Content-Type':'application/json','X-CSRF-TOKEN':_csrf,'Accept':'application/json'},
  ...
  .then(function(data) {
    if (data.success || data.ok) {
      _origType = _selType;
      if (CLIENTS_DATA[_cid]) CLIENTS_DATA[_cid].type = _selType;
      ...
      var dt = document.getElementById('drawerType');
      if (dt) { var tm2 = TYPE_META[_selType]||{icon:'⚪',label:'—'}; dt.textContent = tm2.icon+' '+tm2.label; }
      setTimeout(function() { closeDrawer(); location.reload(); }, 1500);
    } else {
...
function openClientChat(id, name) { if (typeof cwOpen === 'function') { cwOpen(); setTimeout(function() { cwOpenRO(id, true, name); }, 350); } }
function escHtml(t) { return String(t).replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;'); }

document.addEventListener('keydown', function(e) {
  if (e.key === 'Escape') closeDrawer();
  if ((e.key === '/' && !['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)) || (e.ctrlKey && e.key === 'k')) { e.preventDefault(); document.getElementById('clientSearch').focus(); }
});
</script>
<style>@keyframes spin { to { transform: rotate(360deg); } }</style>

<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH /var/www/html/resources/views/super-admin/clients.blade.php ENDPATH**/ ?>